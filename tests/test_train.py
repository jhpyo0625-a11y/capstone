...
from coilvision.eval.metrics import fail_recall, false_reject_rate, fail_scores, macro_f1, per_class_recall
...
def test_coil_dataset_items(tmp_path):
    ...
    for i in range(4):
        name = f"img{i}.png"
        cv2.imwrite(str(tmp_path / name), np.full((size, size, 3), 40 * (i + 1), dtype=np.uint8))
        files.append(name)
    ...
    for train in (False, True):
        ...
        x, y = ds[1]
        assert isinstance(x, torch.Tensor) and x.shape == (3, size, size) and x.dtype == torch.float32
        assert y == 1
        assert len(ds) == 4
